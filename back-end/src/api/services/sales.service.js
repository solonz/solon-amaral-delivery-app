...
const saleReturn = await Sale.create({ user_id: shopCart.id,
...
if (saleReturn) {
  const salesProductPromises = shopCart.shopCart.map(async (product) => {
    await SalesProduct.create({ sale_id: saleReturn.id,
...
} else { throw { status: 404, message: 'Not Found' } }; return saleReturn;
};
